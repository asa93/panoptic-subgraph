...
export function createUserTokenPositionIfNotExisted(id: string, user: User, tokenId: BigInt): TokenPosition {
...
export function handleTokenizedPositionMinted(event: TokenizedPositionMinted): void {
  ...
  let tokenPosition = createUserTokenPositionIfNotExisted(
    ...
    user, event.params.tokenId
  )
  tokenPosition.position = tokenPosition.position + event.params.numberOfContracts
  tokenPosition.save()
...
export function handleTokenizedPositionBurnt(event: TokenizedPositionBurnt): void {
  // Entities can be loaded from the store using a string ID; this ID
  // needs to be unique across all entities of the same type
  let user = createUserIfNotExisted(event.params.recipient.toHex())
  let tokenPosition = createUserTokenPositionIfNotExisted(
...
export function handleTokenizedPositionRolled(event: TokenizedPositionRolled): void {
  // Entities can be loaded from the store using a string ID; this ID
  // needs to be unique across all entities of the same type
  let user = createUserIfNotExisted(event.params.recipient.toHex())
  let oldTokenPosition = createUserTokenPositionIfNotExisted(
    event.params.oldTokenId.toString() + "-" + event.params.recipient.toHex(),
    user, event.params.oldTokenId
  )
  oldTokenPosition.position = oldTokenPosition.position - event.params.numberOfContracts
  ...
  let newTokenPosition = createUserTokenPositionIfNotExisted(
    ...
    user, event.params.newTokenId
  )
  newTokenPosition.position = newTokenPosition.position - event.params.numberOfContracts
